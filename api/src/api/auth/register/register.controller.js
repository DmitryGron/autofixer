...
exports.create = async (req, res, next) => {
	...
	await RegisterModel.findOne({ email: req.body.email }).then(email_exist => {
		...
		if (email_exist) {
			return res.status(409).json(vm.ApiResponse(false, 409, 'email already exist'));
		}
		...
		bcrypt.genSalt(10, (err, salt) => {
			...
			bcrypt.hash(new_user.password, salt, (err, hash) => {
				...
				new_user
					...
					.then(saved => {
						if (!saved) {
							return res.status(400).json(vm.ApiResponse(false, 400, 'an error occur please try again'));
						} else {
							return res.status(201).json(vm.ApiResponse(true, 201, 'registration successful', saved));
						}
					})
					.catch(error => {
...
exports.find = (req, res, next) => {
	RegisterModel.find()
		...
		.then(response => {
			if (!response) {
				return res.status(400).json(vm.ApiResponse(false, 400, 'hoops an error occur unable to find users'));
			} else {
				return res.status(200).json(vm.ApiResponse(true, 200, 'success', response));
			}
		})
		.catch(error => {
			return res.status(500).json(vm.ApiResponse(false, 500, 'hoops an error occur', undefined, error));
		});
};
...
exports.findOne = (req, res, next) => {
	RegisterModel.findOne({ _id: req.params.id })
		.then(found => {
			if (!found) {
				return res.status(400).json(vm.ApiResponse(false, 400, 'unable to find a user with provided id'));
			} else if (found) {
				return res.status(200).json(vm.ApiResponse(true, 200, 'success', found));
			}
		})
		.catch(error => {
			return res.status(500).json(vm.ApiResponse(false, 500, 'hoop internal server error', undefined, error));
		});
};
